...
def answer_question(
    ...
    Returns:
        Dict: A dictionary containing the answer, relevant blocks, and chunk information.
    """
    ...
    # # Calculate similarity scores between answer and relevant blocks
    # answer_embedding = normalize_vector(get_embedding(answer))
    # block_scores = []
    # for block in unique_relevant_blocks:
    #     block_embedding = normalize_vector(get_embedding(block["text"]))
    #     similarity = np.dot(answer_embedding, block_embedding)
    #     block_scores.append((block, similarity))

    # # Sort blocks by similarity score
    # sorted_blocks_with_scores = sorted(block_scores, key=lambda x: x[1], reverse=True)
    # blocks_with_scores = [
    #     {**block, "similarity_score": float(score)}
    #     for block, score in sorted_blocks_with_scores
    # ]

    # logging.info(f"Number of blocks with scores: {len(blocks_with_scores)}")

    # # Prepare and return the final result
    # return {
    #     "answer": answer,
    #     "substrings": substrings,
    #     "blocks": blocks_with_scores,
    # }
...
def answer_question_short_doc(
    ...
    Returns:
        Dict: A dictionary containing the answer, relevant blocks, and substrings.
    """
        ...
        messages=[
            {
                ...
                It is better to provide as much of an answer as possible rather than no answer. 
                """
                # "content": """You are a helpful assistant. You help medical providers answer questions about their patients. Let's think about this step by step. Answer the question based on the provided context of text. Note that the context is a list of texts. Please keep track of which items in the context lists that you use in your answer. Identify the substrings used to answer the question. Return your answer as a dictionary of the following format:
                # {'answer': str. the answer to the question,
                # "substrings": list of strings. a list of substrings that you used to formulate your answer. Be sure to reproduce the text exactly.}""",
            },
            {
                "role": "user",
